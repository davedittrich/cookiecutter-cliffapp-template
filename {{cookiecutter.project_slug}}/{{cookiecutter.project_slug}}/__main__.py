...
# in output of ``{{cookiecutter.project_slug}} help``).
...
def default_environment(default=None):
    ...
    return os.getenv('{{cookiecutter.project_slug.upper()}}', default)
...
class {{cookiecutter.project_slug.capitalize()}}App(App):
    ...
    def build_option_parser(self, description, version):
        ...
        parser.add_argument(
            ...
            dest='data_dir',
            default={{cookiecutter.project_slug.upper()}}_DATA_DIR,
            help=('Root directory for holding data files '
                  f'(Env: ``{{cookiecutter.project_slug.upper()}}_DATA_DIR``; '
                  f'default: { {{cookiecutter.project_slug.upper()}}_DATA_DIR })')
        )
        parser.add_argument(
            '-e', '--elapsed',
            action='store_true',
        ...
        parser.add_argument(
            ...
            help=('Deployment environment selector '
                  '(Env: ``{{cookiecutter.project_slug.upper()}}_ENVIRONMENT``; '
                  f'default: {default_environment()})')
        )
        from {{cookiecutter.project_slug}} import copyright
        parser.epilog = textwrap.dedent(f"""
        For help information on individual commands, use ``{{cookiecutter.project_slug}} <command> --help``.

        Several commands have features that will attempt to open a browser. See
        ``{{cookiecutter.project_slug}} about --help`` to see help information about this feature and how
...
def main(argv=sys.argv[1:]):
    """
    Command line interface for the ``{{cookiecutter.project_slug}}`` project.
    """
...
if __name__ == '__main__':
    ...
    # or just "{{cookiecutter.project_slug}}" result in same argv.
