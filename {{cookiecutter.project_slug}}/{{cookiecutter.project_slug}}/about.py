...
class About(Command):
    """About the ``{{cookiecutter.project_slug}}`` CLI"""
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            dest='readthedocs',
            default=False,
            help=("Open a browser to the ``{{cookiecutter.project_slug}}`` readthedocs page "
                  "(default: False{}).")
        )
        parser = add_browser_options(parser)
        parser.epilog = textwrap.dedent(f"""\
            Shows information about the ``{{cookiecutter.project_slug}}`` CLI.

            ::

                $ {{cookiecutter.project_slug}} about
                {{cookiecutter.project_slug}} version { __version__ }
            ...
            The ``--readthedocs`` option will open a browser to the ``{{cookiecutter.project_slug}}``
    ...
    def take_action(self, parsed_args):
        if parsed_args.readthedocs:
            open_browser(page="https://{{cookiecutter.project_slug}}.readthedocs.io",
            ...
            if (
                ...
                # Hacking formatting for `{{cookiecutter.project_slug}} help` by adding a tab here.
                print(f'{{cookiecutter.project_slug}} version { __version__ }\n\t{ copyright() }')
