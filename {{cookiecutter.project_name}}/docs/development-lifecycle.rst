...
When you build the package with ``setup.py``, or when the ``{{cookiecutter.project_slug}}`` program
wants to get the version number, they both use `setuptools_scm
<https://pypi.org/project/setuptools-scm/>`_.

Someone running ``{{cookiecutter.project_slug}} --version`` after installing the program with
``python -m pip install {{cookiecutter.project_slug}}`` will get a version number similar to
the example above::

    $ {{cookiecutter.project_slug}} --version
    {{cookiecutter.project_slug}} 2021.3.0
...
``{{cookiecutter.project_slug.upper()}}_PYPI_PASSWORD`` on Test PyPI and another named
``{{cookiecutter.project_slug.upper()}}_TEST_PYPI_PASSWORD`` on PyPI and stored them both in *encrypted
...
#. Open a new browser tab to https://test.pypi.org/ and log into your account.
   ...
   then choose **Create a token for {{cookiecutter.project_slug}}**. Enter
   ``{{cookiecutter.project_slug.upper()}}_TEST_PYPI_PASSWORD`` for the token name. For scope, select this
...
#. In the GitHub window, create a new secret named ``{{cookiecutter.project_slug.upper()}}_TEST_PYPI_PASSWORD``.
...
#. Repeat the same process for PyPI using the token name
   ``{{cookiecutter.project_slug.upper()}}_PYPI_PASSWORD``.
